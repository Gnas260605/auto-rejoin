...
config_init_defaults() {
    ...
    AUTO_RESTART_PERIOD="${AUTO_RESTART_PERIOD:-0}"
    ...
    LICENSE_API="${AUTO_REJOIN_LICENSE_API:-${LICENSE_API:-}}"
    ...
    LOW_SERVER_STRICT="${LOW_SERVER_STRICT:-false}"
    LOW_SERVER_PICK_RETRIES="${LOW_SERVER_PICK_RETRIES:-4}"
    LOW_SERVER_PICK_RETRY_DELAY="${LOW_SERVER_PICK_RETRY_DELAY:-2}"
    ALLOW_UNSCOPED_DEEPLINK="${ALLOW_UNSCOPED_DEEPLINK:-false}"
    ALLOW_HOME_FALLBACK="${ALLOW_HOME_FALLBACK:-false}"
...
config_validate() {
    ...
    config_normalize_bool LOW_SERVER_STRICT || { LOW_SERVER_STRICT="false"; status=1; }
    config_validate_uint LOW_SERVER_PICK_RETRIES 4 1 20 || status=1
    config_validate_uint LOW_SERVER_PICK_RETRY_DELAY 2 0 60 || status=1
    config_normalize_bool ALLOW_UNSCOPED_DEEPLINK || { ALLOW_UNSCOPED_DEEPLINK="false"; status=1; }
    config_normalize_bool ALLOW_HOME_FALLBACK || { ALLOW_HOME_FALLBACK="false"; status=1; }
    config_validate_uint WINDOW_MISSING_THRESHOLD 3 1 100 || status=1
    config_normalize_bool WINDOW_REOPEN_ENABLED || { WINDOW_REOPEN_ENABLED="true"; status=1; }
    config_validate_uint UNKNOWN_ACTIVE_WAKE_AFTER 60 1 86400 || status=1
    config_validate_uint UNKNOWN_ACTIVE_WAKE_BACKOFF 60 1 86400 || status=1
    config_validate_uint STALLED_ACTIVE_TIMEOUT 180 1 86400 || status=1
    config_validate_uint HEARTBEAT_STALE_SECONDS 0 0 86400 || status=1
...
config_save() {
...

    {
        ...
        config_write_quoted LICENSE_API
        config_write_raw JOIN_LOW_SERVER
        config_write_raw LOW_SERVER_MIN_PLAYERS
        config_write_raw LOW_SERVER_MAX_PLAYERS
        config_write_raw LOW_SERVER_STRICT
        config_write_raw LOW_SERVER_PICK_RETRIES
        config_write_raw LOW_SERVER_PICK_RETRY_DELAY
        config_write_raw ALLOW_UNSCOPED_DEEPLINK
        config_write_raw ALLOW_HOME_FALLBACK
        config_write_raw WINDOW_MISSING_THRESHOLD
        config_write_raw WINDOW_REOPEN_ENABLED
        config_write_raw UNKNOWN_ACTIVE_WAKE_AFTER
        config_write_raw UNKNOWN_ACTIVE_WAKE_BACKOFF
        config_write_raw STALLED_ACTIVE_TIMEOUT
        config_write_raw HEARTBEAT_STALE_SECONDS
